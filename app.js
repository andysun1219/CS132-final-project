...
app.get("/products/category/:category/product/:product", async (req, res, next) => {
...
app.post("/instock", async (req, res, next) => {
    ...
    try {
        await fs.promises.writeFile("products.json", JSON.stringify(jsonFile), "utf8");
...
app.post("/comments", async (req, res, next)=>{
    ...
    try {
        await fs.promises.writeFile("feedback.json", JSON.stringify(current_comments), "utf8");
...
app.post("/customer", async (req, res, next) => {
    ...
    if (!(firstname && lastname && email && phone)) {
        res.status(CLIENT_ERR_CODE);
        next(Error("Missing POST Parameter: first name, last name, email, or phone"));
    }
    ...
    try {
        current_customers = await fs.promises.readFile("loyalCustomers.json", "utf8");
    }
    catch (err) {
        res.status(SERVER_ERR_CODE);
        err.message = SERVER_ERROR;
    ...
    try {
        await fs.promises.writeFile("loyalCustomers.json", JSON.stringify(current_customers), "utf8");
